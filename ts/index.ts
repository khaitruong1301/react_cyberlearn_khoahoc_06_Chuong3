...
// let arrNumber = new Array<String>();
interface Mang<T> {
    length: number;
    [index: number]: T
}
...
// ReturnType dùng để lấy type(Không phải var,let,const) của kiểu cần lấy => hạn chế sử dụng any(không nhắc lệnh) unkown (không nhắc lệnh)
...
let functionA = () => {
    let result:Action = {type:'a',payload:'123'};
    return result;
}
